...
if((local.Type==1) && (P[j].Type==1)) // only acts between DM particles of type 1 (can easily change if desired)
{
    ...
    double V_i=local.V_i, V_j=get_particle_volume_ags(j), wt_i=V_i, wt_j=V_j, Face_Area_Norm=0, vface_i_minus_j=0; Vec3<double> Face_Area_Vec={0,0,0}, dv={0,0,0}; // calculate densities (in physical units)
    ...
    double igrad[3], jgrad[3], i2grad[3][3], j2grad[3][3], fac_g = All.cf_a3inv/All.cf_atime, fac_g2 = All.cf_a3inv*All.cf_a2inv; Vec3<double> dp={0,0,0}; int m;
    for(k=0;k<3;k++)
    {
