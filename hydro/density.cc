...
#endif
#ifdef GALSF_RESOLVEDISM_DUST
    ASSIGN_ADD(P[i].DGR_around, out->DGR_around, mode);
#endif
...
void density_evaluate_extra_physics_gas(struct INPUT_STRUCT_NAME *local, struct OUTPUT_STRUCT_NAME *out, struct kernel_density *kernel, int j)
{
    ...
    if(local->Type != 0)
    {
        ...
        {
            double DGR_j = 0;
            for(int kd = 0; kd < NUM_RESOLVEDISM_DUST; kd++) DGR_j += CellP[j].Dust[kd];
            out->DGR_around += kernel->mj_wk * DGR_j;
        }
#endif
...
#ifdef GALSF_RESOLVEDISM_DUST
                P[i].DGR_around /= P[i].DensityAroundParticle;
#endif
            }
#endif
