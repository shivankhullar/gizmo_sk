...
int rt_get_source_luminosity(int i, int mode, double *lum, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    {int k; for(k=0;k<N_RT_FREQ_BINS;k++) {lum[k] *= (c_light_code_reduced(i,pp,cell)/C_LIGHT_CODE);}}
    return active_check;
}
...
double c_light_RSL_reductionfactor_local(int i, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    {
        double dp[3]={0},r2=0; for(k=0;k<3;k++) {dp[k] = All.cf_atime*(pp[i].Pos[k] - All.SpecialParticle_Position_ForRefinement[j][k]); r2 += dp[k]*dp[k];}
...
double rt_absorption_rate(int i, int k_freq, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    return (c_light_code_reduced(i,pp,cell)) * rt_absorb_frac_albedo(i,k_freq,pp,cell) * (rt_kappa(i,k_freq,pp,cell) * cell[i].Density*All.cf_a3inv);
}
#endif 
...
{
    return return_flux_limiter(i,k_freq,pp,cell) * c_light_code_reduced(i,pp,cell) / (1.e-45 + cell[i].Rad_Kappa[k_freq] * cell[i].Density*All.cf_a3inv);
}
...
void rt_eddington_update_calculation(int j, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    int k_freq, k; double n_flux_j[3], fmag_j, V_j_inv = cell[j].Density / pp[j].Mass;
    for(k_freq=0;k_freq<N_RT_FREQ_BINS;k_freq++)
    {
        n_flux_j[0]=n_flux_j[1]=n_flux_j[2]=0;
        double flux_vol[3]; for(k=0;k<3;k++) {flux_vol[k] = cell[j].Rad_Flux[k_freq][k] * V_j_inv;}
        ...
        double f_chifac = fmag_j / (MIN_REAL_NUMBER + c_light_code_reduced(j,pp,cell) * cell[j].Rad_E_gamma[k_freq] * V_j_inv);
        ...
        for(k=0;k<6;k++)
        {
            cell[j].ET[k_freq][k] = 0;
            if(k<3)
            {
                cell[j].ET[k_freq][k] = chifac_iso_j + chifac_n_j * n_flux_j[k]*n_flux_j[k];
            } else {
                if(k==3) {cell[j].ET[k_freq][k] = chifac_n_j * n_flux_j[0]*n_flux_j[1];} // recall, for ET: 0=xx,1=yy,2=zz,3=xy,4=yz,5=xz
                if(k==4) {cell[j].ET[k_freq][k] = chifac_n_j * n_flux_j[1]*n_flux_j[2];}
                if(k==5) {cell[j].ET[k_freq][k] = chifac_n_j * n_flux_j[0]*n_flux_j[2];}
            }
        }
    }
    return;
    ...
    int k_freq; for(k_freq=0;k_freq<N_RT_FREQ_BINS;k_freq++) {cell[j].ET[k_freq][0]=cell[j].ET[k_freq][1]=cell[j].ET[k_freq][2]=1./3.; cell[j].ET[k_freq][3]=cell[j].ET[k_freq][4]=cell[j].ET[k_freq][5]=0;}
    return;
#endif
...
void rt_update_driftkick(int i, double dt_entr, int mode, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    for(k_tmp=0; k_tmp<N_RT_FREQ_BINS; k_tmp++)
    {
...
#endif
        {
...
#endif
#ifdef RT_COMOVING
            double ET_dotdot_GradVcom = cell[i].ET[kf][0]*cell[i].Gradients.Velocity[0][0] + cell[i].ET[kf][1]*cell[i].Gradients.Velocity[1][1] + cell[i].ET[kf][2]*cell[i].Gradients.Velocity[2][2]
                + cell[i].ET[kf][3]*(cell[i].Gradients.Velocity[0][1]+cell[i].Gradients.Velocity[1][0]) + cell[i].ET[kf][4]*(cell[i].Gradients.Velocity[2][1]+cell[i].Gradients.Velocity[1][2]) + cell[i].ET[kf][5]*(cell[i].Gradients.Velocity[0][2]+cell[i].Gradients.Velocity[2][0]);
            ...
            dt_e_gamma_band += (c_light_code_reduced(i,pp,cell)/C_LIGHT_CODE) * (-VolP_dotdot_GradV); // account for RSOL term here as usual
#endif
            ...
            if(kf == RT_FREQ_BIN_INFRARED)
            {
                ...
                cell[i].Dust_Temperature = rt_eqm_dust_temp(i, T_gas, total_absorption_rate * vol_inv_phys * C_LIGHT_CODE / c_light_code_reduced(i,pp,cell), pp, cell);
            ...
            // for OTVET/FLD methods, need to apply radiation pressure term here so can limit this b/c just based on a gradient which is not flux-limited [as in hydro operators] //
            {
                ...
                double Sigma_particle = pp[i].Mass / (M_PI*L_particle*L_particle), abs_per_kappa_dt = c_light_code_reduced(i,pp,cell) * (cell[i].Density*All.cf_a3inv) * dt_entr; // effective surface density through particle & fractional absorption over timestep
                ...
                int kx; for(kx=0;kx<3;kx++)
                {
                    ...
                    if(mode==0) {vel_i[kx]=(c_light_code_reduced(i,pp,cell)/C_LIGHT_CODE)*pp[i].Vel[kx]/All.cf_atime;} else {vel_i[kx]=(c_light_code_reduced(i,pp,cell)/C_LIGHT_CODE)*cell[i].VelPred[kx]/All.cf_atime;} // [for comoving] note this is the 'effective' u appearing in the RHD equations for an RSOL, care needed with these factors!
                }
                if(rmag > 0)
                {
                    rmag = sqrt(rmag); for(kx=0;kx<3;kx++) {radacc[kx] /= rmag;} // normalize
                    double rmag_max = de_abs / (pp[i].Mass * c_light_code_reduced(i,pp,cell) * (MIN_REAL_NUMBER + f_kappa_abs)); // limit magnitude to absorbed photon momentum
                    ...
                    for(kx=0;kx<3;kx++)
                    {
                        ...
                        if(mode==0) {pp[i].Vel[kx] += radacc_eff * All.cf_atime;} else {cell[i].VelPred[kx] += radacc_eff * All.cf_atime;}
                    }
                    double d_egy_rad = (2.*f_kappa_abs-1.)*work_band , d_egy_int = -2.*f_kappa_abs*work_band * (C_LIGHT_CODE/c_light_code_reduced(i,pp,cell)); // correct for rsol factor above which reduced vel_i by rsol; -only- add back this term for gas
            ...
            for(k_dir=0;k_dir<3;k_dir++) {if(mode==0) {vel_i[k_dir]=rsol_correction_factor_for_velocity_terms(i,pp,cell)*pp[i].Vel[k_dir]/All.cf_atime;} else {vel_i[k_dir]=rsol_correction_factor_for_velocity_terms(i,pp,cell)*cell[i].VelPred[k_dir]/All.cf_atime;}} // need gas velocity at this time [effective v - note RSOL terms]
            double teqm_inv = cell[i].Rad_Kappa[kf] * rho * c_light_code_reduced(i,pp,cell) + MIN_REAL_NUMBER; // physical code units of 1/time, defines characteristic timescale for coming to equilibrium flux. see notes for CR second-order module for details. //
            eddington_tensor_dot_vector(cell[i].ET[kf], vel_i, vdot_h); // calculate volume integral of scattering coefficient t_inv * (gas_vel . [e_rad*I + P_rad_tensor]), which gives an additional time-derivative term. this is the P term //
    ...
    {
        int k,k_om; double rho=cell[i].Density*All.cf_a3inv, ceff=c_light_code_reduced(i,pp,cell), ctrue=C_LIGHT_CODE, teq_inv=cell[i].Rad_Kappa[kf]*rho*ceff, beta[3], f_a=rt_absorb_frac_albedo(i,kf,pp,cell), f_s=1.-f_a, b_dot_n[N_RT_INTENSITY_BINS]={0}, beta_2=0.;
        ...
        for(i_iter=0; i_iter<n_iter; i_iter++)
        {
            ...
            for(k=0;k<3;k++) {if(mode==0) {beta[k]=pp[i].Vel[k]/(All.cf_atime*ctrue);} else {beta[k]=cell[i].VelPred[k]/(All.cf_atime*ctrue);}} // need gas velocity at this time; with equations written this way, the 'beta' term is the -true- beta, so we have to use the true SOL
            ...
            for(k=0;k<3;k++) {dv_gas[k] = -(flux_f[k]-flux_0[k])/(ceff*pp[i].Mass); v0g=ctrue*beta[k]; ke_gas_0+=(v0g*v0g); ke_gas_f+=(v0g+dv_gas[k])*(v0g+dv_gas[k]);} // note everything is volume-integrated, accounted for above, and we defined flux for convience without the c, so just one power of c here.
            ...
            for(k=0;k<3;k++) {if(mode==0) {pp[i].Vel[k] += dv_gas[k]*All.cf_atime;} else {cell[i].VelPred[k] += dv_gas[k]*All.cf_atime;}} // update gas velocities (radiation pressure forces here)
...
#else
    double mom_fac = 1. - rsol_correction_factor_for_velocity_terms(i,pp,cell) * total_erad_emission_minus_absorption / (pp[i].Mass * c_light_code_reduced(i,pp,cell)*c_light_code_reduced(i,pp,cell)); // back-reaction on gas from emission, which is isotropic in the fluid frame but anisotropic in the lab frame. this effect is only important in actually semi-relativistic problems so we use "real" C here, not a RSOL, and match the corresponding term above in the radiation flux equation (if that is evolved explicitly). careful checking-through gives the single termm here, not both
    ...
    {int k_dir; for(k_dir=0;k_dir<3;k_dir++) {if(mode==0) {pp[i].Vel[k_dir] *= mom_fac;} else {cell[i].VelPred[k_dir] *= mom_fac;}}}
#endif
...
void rt_apply_boundary_conditions(int i, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    get_background_isrf_urad(i, urad, pp, cell);
...
void rt_set_simple_inits(int RestartFlag)
{
    ...
    int i; for(i = 0; i < NumPart; i++)
    {
        if(P[i].Type == 0)
        {
            ...
            get_background_isrf_urad(i, urad, P, CellP);
#endif
            for(k = 0; k < N_RT_FREQ_BINS; k++)
            {
...
#if defined(RT_EVOLVE_FLUX)
                CellP[i].Rad_Flux_Pred[k][2]=CellP[i].Rad_Flux[k][2] = E_cell_thin*c_light_code_reduced(i,P,CellP);
...
double dust_dE_cooling(int i, double Tgas, double Tdust, double* Tdust_fixedpoint_1, double* Tdust_fixedpoint_2, struct particle_data *pp, struct gas_cell_data *cell){
    ...
    double lambda_to_dErad = (c_light_code_reduced(i, pp, cell)/C_LIGHT_CODE) * nHcgs * nHcgs * (dt*UNIT_TIME_IN_CGS) / (cell[i].Density * All.cf_a3inv * UNIT_DENSITY_IN_CGS) / (UNIT_SPECEGY_IN_CGS) * pp[i].Mass; /* need to account for RSOL factors in emission/absorption rates */
    ...
    double fac_emission = 4.*5.67e-5/(UNIT_PRESSURE_IN_CGS*UNIT_VEL_IN_CGS)*pp[i].Mass*(c_light_code_reduced(i,pp,cell)/C_LIGHT_CODE)*dt;
    ...
    dust_absorption += e_IR_final * c_light_code_reduced(i,pp,cell) * rt_kappa_adaptive_IR_band(i, Tdust, T_IR_final,-1,1, pp, cell) * cell[i].Density*All.cf_a3inv * dt;
