...
void cooling_parent_routine(void)
{
    ...
    for (i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        ...
        if((cell[i].Density*All.cf_a3inv > All.PhysDensThresh) && ((All.ComovingIntegrationOn==0) || (cell[i].Density>=All.OverDensThresh))) {continue;} /* no cooling for effective-eos star-forming particles */
#endif
#ifdef GALSF_FB_TURNOFF_COOLING
        if(cell[i].DelayTimeCoolingSNe > 0) {continue;} /* no cooling for particles marked in delayed cooling */
#endif
...
#pragma omp for schedule(dynamic)
    ...
    for(j=0;j<N_active;j++)
    {
        ...
        do_the_cooling_for_particle(i, P, CellP); /* do the actual cooling */
    }
...
void do_the_cooling_for_particle(int i, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    if((dtime>0)&&(pp[i].Mass>0)&&(pp[i].Type==0))  // upon start-up, need to protect against dt==0 //
    {
        ...
        for(k = 0; k < 3; k++)
        {
            grav_acc = All.cf_a2inv * pp[i].GravAccel[k];
#ifdef PMGRID
            grav_acc += All.cf_a2inv * pp[i].GravPM[k];
#endif
            DtInternalEnergyEffCGS -= cell[i].GravWorkTerm[k] * All.cf_atime * grav_acc;
        }
#endif
...
#ifndef CHIMES
            cell[i].Ne = 1.0 + 2.0*yhelium(i, pp, cell); /* fully ionized. note that this gives Ne as free electron fraction per H */
#endif
        }
#endif
        ...
        double nHcgs = nH_cgs(i, pp, cell); /* hydrogen number dens in cgs units */
        double ratefact = (c_light_code_reduced(i, pp, cell)/C_LIGHT_CODE) * nHcgs * nHcgs / (cell[i].Density * All.cf_a3inv * UNIT_DENSITY_IN_CGS) * (dtime*UNIT_TIME_IN_CGS) / (UNIT_SPECEGY_IN_CGS) * pp[i].Mass; /* need to account for RSOL factors in emission/absorption rates */
            ...
            if((fabs(cell[i].Lambda_RadiativeCooling_toRHDBins[k]) > MIN_REAL_NUMBER) && (fabs(de_rad_tot) > MIN_REAL_NUMBER))
            {
                ...
                if(fabs(de_rad) > MIN_REAL_NUMBER)
                {
                    ...
                    if(fabs(de_rad) > MIN_REAL_NUMBER)
                    {
                        ...
                        for(kv=0;kv<3;kv++) {if(corrfac > 0) {cell[i].Rad_Flux[k][kv] *= corrfac; cell[i].Rad_Flux_Pred[k][kv] *= corrfac;} else {double fluxfac = rsol_correction_factor_for_velocity_terms(i, pp, cell)*cell[i].VelPred[kv]/All.cf_atime * de_rad; cell[i].Rad_Flux[k][kv] += fluxfac; cell[i].Rad_Flux_Pred[k][kv] += fluxfac;}}
#endif
                        double momfac = 1. - de_rad / (pp[i].Mass * C_LIGHT_CODE*c_light_code_reduced(i, pp, cell)); // back-reaction on gas from emission [note peculiar units here, its b/c of how we fold in the existing value of v and tilde[u] in our derivation - one rsol factor in denominator needed]
                        for(kv=0;kv<3;kv++) {pp[i].Vel[kv] *= momfac; cell[i].VelPred[kv] *= momfac;}
                    }
                }
            }
        }
...
double convert_u_to_temp(double u, double rho, int target, double *ne_guess, double *nH0_guess, double *nHp_guess, double *nHe0_guess, double *nHep_guess, double *nHepp_guess, double *mu_guess, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    *mu_guess = Get_Gas_Mean_Molecular_Weight_mu(temp_guess, rho, nH0_guess, ne_guess, 0., target, pp, cell); // get mu with that temp
    ...
    do
    {
        ...
        find_abundances_and_rates(log10(temp, pp, cell), rho, target, -1, 0, ne_guess, nH0_guess, nHp_guess, nHe0_guess, nHep_guess, nHepp_guess, mu_guess, &Lambda_filler, &Lambda_filler, &Lambda_filler, &Lambda_filler); // all the thermo variables for this T
...
double find_abundances_and_rates(double logT, double rho, int target, double shieldfac, int return_cooling_mode,
                                 ...
                                 double *mu_guess, double *LambdaExc_return, double *LambdaIon_return, double *LambdaRec_return, double *LambdaFF_return, struct particle_data *pp, struct gas_cell_data *cell)
{
    int j, jp, niter;
    ...
    {
        nH0 = 1.0; nHe0 = yhelium(target, pp, cell); nHp = 0; nHep = 0; nHepp = 0; n_elec = 1.e-22;
    ...
    {
        nH0 = 0; nHe0 = 0; nHp = 1.0; nHep = 0; nHepp = yhelium(target, pp, cell); n_elec = nHp + 2.0 * nHepp;
        *nH0_guess=nH0; *nHe0_guess=nHe0; *nHp_guess=nHp; *nHep_guess=nHep; *nHepp_guess=nHepp; *ne_guess=n_elec;
    ...
    do
    {
        ...
        if( ((gJHe0ne + geHe0) <= MIN_REAL_NUMBER) || (aHepp <= MIN_REAL_NUMBER) ) 	/* no ionization at all */
        {
            nHep = 0.0;
            nHepp = 0.0;
            nHe0 = yhelium(target, pp, cell);
        }
        else
        {
            nHep = yhelium(target, pp, cell) / (1.0 + (aHep + ad) / (geHe0 + gJHe0ne) + (geHep + gJHepne) / aHepp);	/* eqn (35) */
        ...
        if(target >= 0)
        {
            double yHe = yhelium(target, pp, cell); // will use helium fraction below
	...
	if(bisection_mode) { // if fixed-point mode is not converging fast enough and we switched to bisection mode
        ...
        if(niter > (MAXITER - 10)) {printf("n_elec= %g/%g/%g yh=%g nHcgs=%g niter=%d\n", n_elec,neold,nenew, yhelium(target, pp, cell), nHcgs, niter);}
    }
    while(niter < MAXITER);

    if(niter >= MAXITER) {printf("failed to converge in find_abundances_and_rates(): logT_input=%g  rho_input=%g  ne_input=%g target=%d ID=%ld shieldfac=%g cooling_return=%d", logT_input, rho_input, ne_input, target, (long)pp[target].ID, shieldfac, return_cooling_mode); endrun(13);}
...
// #endif
            ...
            double gradv_norm_kms_pc = velocity_gradient_norm(target, pp, cell) * UNIT_VEL_IN_KMS / UNIT_LENGTH_IN_PC; // velocity gradient in km/s/pc
  ...
  else				/* here we're outside of tabulated rates, T>Tmax K */
    {
        ...
        nHp = 1.0; nHep = 0; nHepp = yhelium(target, pp, cell); n_elec = nHp + 2.0 * nHepp; /* very hot: H and He both fully ionized */
        *n_elec_eval = n_elec; /* save this value for the output cycle */
...
{   /* include local self-shielding with the following */
    int i; for(i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        if(pp[i].Type==0)
        {
            if((cell[i].Rad_Flux_UV>0) && (pp[i].KernelRadius>0) && (cell[i].Density>0) && (pp[i].Mass>0) && (All.Time>0))
            {
                cell[i].Rad_Flux_UV *= UNIT_FLUX_IN_CGS * 1276.19; cell[i].Rad_Flux_EUV *= UNIT_FLUX_IN_CGS * 1276.19; // convert to Habing units (normalize strength to local MW field in this [narrow] band, so not the 'full' Habing flux)
                double surfdensity = evaluate_NH_from_GradRho(pp[i].GradRho,pp[i].KernelRadius,cell[i].Density,pp[i].NumNgb,1,i); // in CGS
                double tau_nuv = rt_kappa(i,RT_FREQ_BIN_FIRE_UV,pp,cell) * surfdensity; // optical depth: this part is attenuated by dust //
#if (GALSF_FB_FIRE_STELLAREVOLUTION <= 2)
                tau_nuv *= (1.0e-3 + (pp[i].Metallicity[0]/All.SolarAbundances[0])*return_dust_to_metals_ratio_vs_solar(i,0, pp, cell)); // if using older FIRE defaults, this was manually added instead of rolled into rt_kappa -- annoying but here for completeness //
#endif
                double tau_euv = 3.7e6 * surfdensity * UNIT_SURFDEN_IN_CGS; // optical depth: 912 angstrom kappa_euv: opacity from neutral gas //
                cell[i].Rad_Flux_UV *= exp(-DMIN(tau_nuv,90.)); // attenuate [important in newer modules depending on UV flux to fully-attenuate down to << 1e-6 in dense gas]
                //cell[i].Rad_Flux_UV *= 0.01 + 0.99/(1.0 + 0.8*tau_nuv + 0.85*tau_nuv*tau_nuv); // attenuate (for clumpy medium with hard-minimum 1% scattering)
                //cell[i].Rad_Flux_EUV *= exp(-DMIN(tau_euv,90.)); // attenuate [important in newer modules depending on UV flux to fully-attenuate down to << 1e-6 in dense gas]
                cell[i].Rad_Flux_EUV *= 0.01 + 0.99/(1.0 + 0.8*tau_euv + 0.85*tau_euv*tau_euv); // attenuate (for clumpy medium with hard-minimum 1% scattering) //
            } else {cell[i].Rad_Flux_UV = cell[i].Rad_Flux_EUV = 0;}
            ...
            {   /* assign typical strong HII region flux + enough flux to maintain cell fully-ionized, regardless (x'safety-factor') */
                double n1000 = cell[i].Density*All.cf_a3inv*UNIT_DENSITY_IN_NHCGS / 1000.; // density in 1000 cm^-3
                ...
                cell[i].Rad_Flux_UV += flux_compactHII; cell[i].Rad_Flux_EUV += flux_compactHII;
            }
#endif
        }
    }
...
double ThermalProperties(double u, double rho, int target, double *mu_guess, double *ne_guess, double *nH0_guess, double *nHp_guess, double *nHe0_guess, double *nHep_guess, double *nHepp_guess, struct particle_data *pp, struct gas_cell_data *cell)
{
    ...
    *mu_guess = Get_Gas_Mean_Molecular_Weight_mu(temp, rho, nH0_guess, ne_guess, 0, target, pp, cell);
    ...
    if(target >= 0) {if(cell[target].DelayTimeHII > 0) {cell[target].Ne = 1.0 + 2.0*yhelium(target, pp, cell); *nH0_guess=0; nHe0_guess=0;}} /* fully ionized [if using older model] */
    *mu_guess = Get_Gas_Mean_Molecular_Weight_mu(temp, rho, nH0_guess, ne_guess, 0, target, pp, cell);
#endif
    return temp;
#endif
